...
export default function App() {
 ...
 const [workbookReplacement, setWorkbookReplacement] = useState(false);
 ...
 async function connect() {
  ...
  try {
   ...
   provider.disconnect(); setProvider(connected); setConsent(false); setReview(false); setWorkbookReplacement(false); setPanel(null);
   if (sameAccount) {
 ...
 function useDemo(nextScenario = scenario) {
  setWorkbookReplacement(false); provider.disconnect(); const nextPreferences = { ...DEFAULT_PREFERENCES }; const nextDate = tomorrowDate(nextPreferences.timeZone); setPreferences(nextPreferences); setDate(nextDate); const next = createDemoProvider(nextScenario); setProvider(next); setScenario(nextScenario); setActivity([]); setApiKey(''); setConsent(false); setPanel(null); void build(next, nextDate, nextPreferences, false);
 }
 async function recoverWorkbook(allowReplacement = false) {
  if (!provider.recoverWorkbook) return;
  setBusy(true); setError(''); setNotice('');
  try {
   const outcome = await provider.recoverWorkbook(allowReplacement);
   setWorkbookReplacement(outcome === 'confirmation_required');
   if (outcome !== 'confirmation_required') setNotice(outcome === 'recovered' ? 'Found your existing workbook. Retry the approved save or action to finish.' : outcome === 'reset' ? 'Recovery marker cleared. Your next approved save or action can create a replacement workbook.' : 'No unresolved workbook creation was found.');
  } catch (e) { setWorkbookReplacement(false); setError(errorText(e)); }
  finally { setBusy(false); }
 }
 ...
 return <div className="app-shell">
  ...
  <div className="workspace">
   ...
   <main id="main">
    ...
    <aside className="day-aside" aria-label="Schedule and evidence"><section className="schedule-panel"><div className="section-heading"><h2>A glance at your day</h2><CalendarDays size={17}/></div><p className="tiny">{preferences.timeZone.replaceAll('_', ' ')}</p><div className="timeline">{plan?.events.length ? [...plan.events].sort((a, b) => a.start.localeCompare(b.start)).map(event => <div className={`timeline-event ${event.isDayweave ? 'dayweave-event' : ''}`} key={event.id}><span className="timeline-point"/><time>{event.allDay ? 'ALL DAY' : fullTime(event.start, plan.timeZone)}</time><strong>{event.title}</strong><small>{event.allDay ? 'All-day commitment' : `${time(event.start, plan.timeZone)} – ${time(event.end, plan.timeZone)}`}</small></div>) : <div className="empty-schedule"><CalendarDays size={25}/><p>{busy ? 'Reading your day…' : 'Some lovely open space.'}</p><small>{busy ? 'Your commitments will appear here.' : 'No calendar commitments found for this date.'}</small></div>}</div><div className="schedule-footer"><span className="source-dot calendar"/>{provider.mode === 'demo' ? 'Demo Calendar' : 'Google Calendar'}<span>{plan?.events.length ?? 0} events</span></div></section>
     <section className="agent-panel"><div className="section-heading"><h2>Behind the weave</h2><Sparkles size={17}/></div><span className={`intelligence-badge ${plan?.aiUsed ? 'ai-active' : ''}`}>{plan?.aiUsed ? 'Gemini AI used' : busy ? 'Preparing your plan' : 'Planning without AI'}</span><p>{plan?.aiUsed ? 'AI helped connect the dots. Validated scheduling rules set the times.' : 'Your plan uses your calendar and routines. AI assistance is off for this plan.'}</p><details className="trace-details"><summary>View agent activity<span>{trace.length} steps</span></summary><div className="trace-list" aria-live="polite">{trace.length ? trace.map(entry => <div key={entry.id} className={`trace-entry ${entry.status}`}><span>{entry.status === 'running' ? <LoaderCircle size={13} className="spinning"/> : entry.status === 'success' ? <Check size={13}/> : <span>·</span>}</span><div><strong>{entry.tool.replaceAll('_', ' ')}</strong><p>{entry.message}</p></div></div>) : <p className="tiny">Activity appears as your plan is prepared.</p>}</div></details></section>
  ...
  {panel === 'connect' && <Modal title="Bring your day together." onClose={() => setPanel(null)} busy={totalBusy}>
   ...
   {provider.mode === 'live' ? <div className="connected-notice"><ShieldCheck size={20}/><div><strong>Your Google account is connected</strong><p>Each app’s current status is shown in the sidebar.</p><button className="text-button" disabled={totalBusy} onClick={() => void connect()}>Reconnect Google</button>{provider.recoverWorkbook && <button className="text-button" disabled={totalBusy} onClick={() => void recoverWorkbook()}>Recover workbook</button>}<button className="text-button" disabled={totalBusy} onClick={() => useDemo()}><LogOut size={14}/>Disconnect & return to demo</button></div></div> : <><label className="field">Google OAuth client ID<input value={clientId} onChange={e => setClientId(e.target.value)} autoComplete="off" placeholder="…apps.googleusercontent.com"/></label><p className="tiny">A public Web application client ID from your Google Cloud project. Enable Gmail, Calendar, Sheets and Drive APIs and allow this site’s origin. <a href={`${import.meta.env.BASE_URL}setup.html`} target="_blank" rel="noreferrer">Setup guide ↗</a></p><button className="button google-button" disabled={totalBusy || !clientId.trim()} onClick={() => void connect()}>{busy ? <LoaderCircle className="spinning" size={18}/> : <span className="google-g">G</span>}Connect Google</button></>}
   {workbookReplacement && <div className="notice"><p>No workbook was found for the interrupted creation. Starting over could create an extra workbook if the original appears later. Confirm only after checking Google Drive. No workbook is created now.</p><button className="button secondary" disabled={totalBusy} onClick={() => setWorkbookReplacement(false)}>Keep waiting</button><button className="button primary" disabled={totalBusy} onClick={() => void recoverWorkbook(true)}>Confirm replacement on next save</button></div>}
   ...
   {error && <p className="notice" role="alert">{error}</p>}{notice && <p className="notice" role="status">{notice}</p>}<p className="tiny setup-privacy"><ShieldCheck size={13}/>No secrets are saved to local storage. <a href={`${import.meta.env.BASE_URL}privacy.html`} target="_blank" rel="noreferrer">Privacy details</a></p>
  </Modal>}
  {panel === 'activity' && <Modal title="The little things, taken care of." onClose={() => setPanel(null)} wide><p className="muted">{provider.mode === 'demo' ? 'Local demo activity. No changes have been made to your Google account.' : 'Approved actions recorded in your Google Sheets activity log.'}</p>{activity.length ? <div className="activity-list">{activity.map((row, index) => <div key={`${row.actionId}-${index}`}><span className="activity-check"><Check size={17}/></span><div><strong>{row.title}</strong><p>{fullTime(row.start, preferences.timeZone)} · {row.status}</p>{safeUrl(row.eventUrl) && <a href={safeUrl(row.eventUrl)} target="_blank" rel="noreferrer">View event ↗</a>}</div></div>)}</div> : <div className="empty-activity"><Unplug size={32}/><h3>Nothing added yet.</h3><p>Choose a suggestion and review it. Approved actions will appear here.</p></div>}{safeUrl(provider.getSpreadsheetUrl()) && <a className="button secondary" href={safeUrl(provider.getSpreadsheetUrl())} target="_blank" rel="noreferrer"><Sheet size={16}/>Open activity workbook<ArrowUpRight size={14}/></a>}</Modal>}
  {panel === 'about' && <Modal title="A day woven around you." onClose={() => setPanel(null)}><div className="how-it-works"><div><Mail/><h3>Notice your rhythm</h3><p>Read relevant receipts, calendar commitments and your saved routines. Email delivery times are never treated as consumption times.</p></div><div><Moon/><h3>Make a thoughtful plan</h3><p>Find time for rest, coffee and lunch around your commitments. Review the evidence behind each suggestion, and explore food ideas if you choose a food goal.</p></div><div><Coffee/><h3>Choose the little changes</h3><p>Select what fits and review the exact events. Once you approve, Dayweave adds them to Calendar and records the results in Sheets.</p></div></div></Modal>}
 </div>;
}
